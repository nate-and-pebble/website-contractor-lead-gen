...
function formatFollowUp(dateStr: string | null): { label: string; overdue: boolean } | null {
  ...
  if (diffDays < 0) {
    ...
    return { label: absDays === 1 ? "1 day overdue" : `${absDays} days overdue`, overdue: true };
  }
...
export function CallQueue({ contacts, selectedId, onSelect }: CallQueueProps) {
  if (contacts.length === 0) {
    return (
      <div className="flex w-80 shrink-0 flex-col items-center justify-center border-r border-zinc-200 bg-white p-8 text-center">
        ...
        <p className="mt-3 text-sm font-medium text-zinc-700">No calls queued</p>
